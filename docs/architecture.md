...
│   ├── blog.api.ts         # Public blog + image fetch functions
│   ├── comment.api.ts      # Comment submission
│   └── admin.api.ts        # Admin auth + blog/comment management
│
├── hooks/
...
- **Pagination N+1**: `getBlogsWithCover` makes one extra request per blog for the cover image. The backend should add a `coverImageUrl` field to the blog list response to eliminate this.
